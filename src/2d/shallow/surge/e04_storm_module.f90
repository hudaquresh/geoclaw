...
! ==============================================================================
module slosh_storm_module

    implicit none
    save

    ! Holland storm type definition
    type slosh_storm_type
...
contains
    ...
    ! Setup routine for the slosh model
    subroutine set_slosh_storm(storm_data_path, storm, log_unit)
        ...
        type(slosh_storm_type), intent(in out) :: storm
    ...
    end subroutine set_slosh_storm
    ...
    ! ==========================================================================
    !  slosh_storm_location(t,storm)
    !    Interpolate location of hurricane in the current time interval
    ! ==========================================================================
    function slosh_storm_location(t,storm) result(location)
        ...
        call get_slosh_storm_data(t, storm, location, junk, junk(1),        &
                                    junk(1), junk(1), junk(1))

    end function slosh_storm_location

    ! ==========================================================================
    !  slosh_storm_direction
    !   Angle off of due north that the storm is traveling
    ! ==========================================================================
    real(kind=8) function slosh_storm_direction(t, storm) result(theta)

        implicit none

        ! Input
        real(kind=8), intent(in) :: t
        type(slosh_storm_type), intent(in) :: storm

        ! Locals
        real(kind=8) :: junk(2), velocity(2)

        ! Fetch velocity of storm which has direction encoded in it
        call get_slosh_storm_data(t, storm, junk, velocity, junk(1),  &
                                    junk(1), junk(1), junk(1))

        ! Unit directional vector
        theta = atan2(velocity(2),velocity(1))

    end function slosh_storm_direction
    ...
    ! ==========================================================================
    !  get_slosh_storm_data()
    !    Interpolates in time and returns storm data.
    ! ==========================================================================
    subroutine get_slosh_storm_data(t, storm, location, velocity, &
                                                max_wind_radius,    &
                                                max_wind_speed,     &
                                                central_pressure,   &
                                                rrp)
        ...
        type(slosh_storm_type), intent(in) :: storm   ! Storm
    ...
    end subroutine get_slosh_storm_data

    
    ! ==========================================================================
    !  set_holland_storm_fields()
    ! ==========================================================================
    subroutine set_slosh_storm_fields(maux,mbc,mx,my,xlower, &
        ...
        call get_slosh_storm_data(t,storm,sloc,tv,mwr,mws,Pc,rrp)
...
!       # Holland 2010 Model 
        ...
        do j=1-mbc,my+mbc
            ...
            do i=1-mbc,mx+mbc
                ...
                wind = ((2 * mwr * mws + f * mwr**2) * r) / (mwr**2 + r**2) & 
                        - (f * r) / 2 
    ...
    end subroutine set_slosh_storm_fields

end module slosh_storm_module
