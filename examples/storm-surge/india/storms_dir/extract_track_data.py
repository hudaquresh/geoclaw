...
#AL, 09, 2008090412,   , BEST,   0,  230N,  564W, 120,  938, HU,  64, NEQ,   40,
#30,   25,   30, 1009,  180,  15, 145,   0,   L,   0,    ,   0,   0,        IKE,
#D, 12, NEQ, 360, 270, 120, 300
...
# "        DDDDDDDDDD      XXXX  XXX  DDDDS  DDDDS DDD" % 
        ...
        # "(8x      i4  i2i2i26x    a4  2xi3 xi4  a i4   a i3
        # 2x,i4,47x,i3,2x,i3)"
        # "(        YYYYMMDDHH      BEST  FOR lat_D long_D max  cpre
        # rrp  rad"
        for n in xrange(len(storm['track'][0])):
            # AL, 00, HHHHMMDDHH,   , BEST,  0, LATN, LONW, XX, CPRE, TS,  , 0,
            # 0, 0, 0,
...
#def extract_data(path, mask_dist=numpy.infty, mask_ca:
...
#    path = "/Users/hudaqureshi/research/scratch/Mumbai_IO_ncep_reanal.mat"
    #if len(sys.argv) > 1:
    #    path = sys.argv[1]
...
#    create_storm_file(storms[-2], output_path="mumbai_2.storm")
    ...
    for x in range(0,len(storms)):  
        ...
        #fig.savefig('track/track_%i.pdf' %x)
        storm_num = str(x)
        storm_num = storm_num.rjust(4,'0') 
        create_storm_file(storms[x], output_path='mumbai_storm_dir/mumbai_%s.storm' %storm_num)
