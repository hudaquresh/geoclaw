...
module cle_storm_module
    ...
    type cle_storm_type 
        ...
        ! These paramters are located at time points but are interpolated 
        ! in time and space when the relevant fields are requested. 
    ...
    ! real(kind=8), private :: A,B #TODO: Erase, temp kept for holland comparison 
...
contains
    ...
    subroutine set_cle_storm_fields(maux,mbc,mx,my,xlower, &
        ...
        if (last_time /= t) then
           ...
           last_time = t
           call solve_hurricane_wind_parameters(f, r_max, v_max, res, r_0, r_a)
           dr = real(r_0)/(res - 1)
           ...
           do j=2, res
              ...
              if(j <= in_res) then
                 r = (j - 1)*dr
                 v_vec(j) = evaluate_v_a(f, r_max, v_max, r)
              else
