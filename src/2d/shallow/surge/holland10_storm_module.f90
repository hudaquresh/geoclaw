...
! ==============================================================================
module holland10_storm_module

    implicit none
    save

    ! Holland storm type definition
    type holland10_storm_type
    ...
    end type holland10_storm_type

    logical, private :: module_setup = .false.

    ! Interal tracking variables for storm
    real(kind=8), private :: A,B,Bs
...
contains
    ...
    ! Setup routine for the holland10 model
    subroutine set_holland10_storm(storm_data_path, storm, log_unit)
        ...
        type(holland10_storm_type), intent(in out) :: storm
    ...
    end subroutine set_holland10_storm
    ...
    ! ==========================================================================
    !  holland10_storm_location(t,storm)
    !    Interpolate location of hurricane in the current time interval
    ! ==========================================================================
    function holland10_storm_location(t,storm) result(location)
        ...
        call get_holland10_storm_data(t, storm, location, junk, junk(1),        &
                                    junk(1), junk(1), junk(1))

    end function holland10_storm_location

    ! ==========================================================================
    !  holland10_storm_direction
    !   Angle off of due north that the storm is traveling
    ! ==========================================================================
    real(kind=8) function holland10_storm_direction(t, storm) result(theta)

        implicit none

        ! Input
        real(kind=8), intent(in) :: t
        type(holland10_storm_type), intent(in) :: storm

        ! Locals
        real(kind=8) :: junk(2), velocity(2)

        ! Fetch velocity of storm which has direction encoded in it
        call get_holland10_storm_data(t, storm, junk, velocity, junk(1),  &
                                    junk(1), junk(1), junk(1))

        ! Unit directional vector
        theta = atan2(velocity(2),velocity(1))

    end function holland10_storm_direction
    ...
    ! ==========================================================================
    !  get_holland10_storm_data()
    !    Interpolates in time and returns storm data.
    ! ==========================================================================
    subroutine get_holland10_storm_data(t, storm, location, velocity, &
                                                max_wind_radius,    &
                                                max_wind_speed,     &
                                                central_pressure,   &
                                                rrp)
        ...
        type(holland10_storm_type), intent(in) :: storm   ! Storm
    ...
    end subroutine get_holland10_storm_data

    
    ! ==========================================================================
    !  set_holland_storm_fields()
    ! ==========================================================================
    subroutine set_holland10_storm_fields(maux,mbc,mx,my,xlower, &
        ...
        real(kind=8) :: rn, dg, edg, rg, vn, xn, xx 
        integer :: i,j

        ! Get interpolated storm data
        call get_holland10_storm_data(t,storm,sloc,tv,mwr,mws,Pc,rrp)
...
!       # Holland 2010 Model 
        ...
        !! Using equation 6 without mws 
        !dg = (mwr/rrp)**B 
        !edg = exp(-1.d0*dg) 
        !rg = B * dp * dg * edg/rho_air 
        !rn = rrp  
        !vn = 5  
        !xn = log(vn)/log(rg) 
        ...
        if (DEBUG) print "('Ambient Pressure (Pa) = ',d16.8)",Pa
        if (DEBUG) print "('Central Pressure (Pc) = ',d16.8)",Pc 
        if (DEBUG) print "('Pa - Pc = ',d16.8)",dp 
        if (DEBUG) print "('dg = ',d16.8)",dg 
        if (DEBUG) print "('Pa - Pc = ',d16.8)",dp 
...
!!         aux(pressure_index,:,:) = Pa
        ...
        do j=1-mbc,my+mbc
            ...
            do i=1-mbc,mx+mbc
                ...
                !! # HOLLAND 1980 WIND SPEED CALCULATION 
                !! Speed of wind at this point
                !wind = sqrt((mwr / r)**B &
                !        * exp(1.d0 - (mwr / r)**B) * mws**2.d0 &
                !        + (r * f)**2.d0 / 4.d0) - r * f / 2.d0
    ...
    end subroutine set_holland10_storm_fields

end module holland10_storm_module
...
!        real(kind=8) :: x, y, r, theta, sloc(2)
!        real(kind=8) :: f, mwr, mws, Pc, Pa, dp, wind, tv(2), rrp
!        real(kind=8) :: dg, edg, rg, rn, vn, xn, xx   
!        real(kind=8) :: mod_mws, trans_speed, ramp
!        integer :: i,j
!
!        ! Get interpolated storm data
...
!
!        ! Calculate Holland parameters
!        Bs = (mod_mws**2.d0) * (rho_air * exp(1.d0))/(100*dp) 
!        Bs = (mws**2.d0) * (rho_air * exp(1.d0))/(100*dp) 
!    
!        dg = (mwr/rrp)**Bs 
!        edg = exp(-1.d0*dg) 
!        rg = 100 * Bs * dp * dg * edg/rho_air 
!        rn = rrp  
!        vn = 12  
!        xn = log(vn)/log(rg) 
!
...
!
!                ! Set pressure field
!                aux(pressure_index,i,j) = Pc + dp * exp(-(mwr / r)**Bs)
!
!                ! Speed of wind at this point
!                if (r <= mwr) then 
!                    wind = mws * (((mwr/r)**Bs) * exp(1-(mwr/r)**Bs))**0.5 
!                else 
!                    !xx = 0.5 + (r - mwr) * (xn - 0.5)/(rn - mwr)
!                    !print *,'xx', xx
!                    !xx = 0.6 * (1 - dp/215) 
!                    xx = 0.5
!                    wind = mws * (((mwr/r)**Bs) * exp(1-(mwr/r)**Bs))**xx  
!                end if
!
!                ! Convert wind velocity from top of atmospheric boundary layer
!                ! (which is what the Holland curve fit produces) to wind
...
!        real(kind=8) :: f, mwr, mws, Pc, Pa, dp, wind, tv(2), rrp
!        real(kind=8) :: mod_mws, trans_speed, ramp
!        real(kind=8) :: xx, xn, rn, vn, dg, edg, rg 
!        integer :: i,j
!
!        ! Get interpolated storm data
!        call get_holland10_storm_data(t,storm,sloc,tv,mwr,mws,Pc,rrp)
